    ...
    if movie_soup is not None:
        ...
        if page_section is not None:
            ...
            if len(divs) > 1:
                ...
                # Se o título já foi processado, ignore
                if title in processed_titles:
                    return
                
                # Marcar o título como processado
                ...
                # Escrever os dados no arquivo CSV com um contador (current_id)
                with open('movies.csv', mode='a', newline='', encoding='utf-8') as file:
                    movie_writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
                    if all([title, date, rating, plot_text]):
                        print(f"{current_id}. {title}, {date}, {rating}, {plot_text}")
                        movie_writer.writerow([current_id, title, date, rating, plot_text])
...
def main():
    ...
    end_time = time.time()
    print('Tempo total de execução: ', end_time - start_time)
